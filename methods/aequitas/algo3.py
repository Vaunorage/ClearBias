...
class Local_Perturbation(object):
    ...
    def __call__(self, x):
        ...
        ei = check_for_error_condition(self.conf, self.preds, x)

        # update the probabilities of directions
        if (ei != int(x[self.sensitive_param - 1]) and direction_choice == -1) or (
                not (ei != int(x[self.sensitive_param - 1])) and direction_choice == 1):
            self.direction_probability[param_choice] = min(self.direction_probability[param_choice] +
                                                           ...
                                                           1)
        elif (not (ei != int(x[self.sensitive_param - 1])) and direction_choice == -1) or (
                ei != int(x[self.sensitive_param - 1]) and direction_choice == 1):
...
def check_for_error_condition(conf: DiscriminationData, preds, t):
    ...
    test_cases_df['outcome'] = preds(test_cases_df.to_numpy())

    max_discrimination = (test_cases_df['outcome'] - original_pred).max()

    return max_discrimination


def aequitas(dataset: DiscriminationData, sensitive_param, max_global, max_local, step_size, iter):
    """
    ...
    def evaluate_local(inp):
        result = check_for_error_condition(dataset, preds, inp)
