from biasscan.generator import get_entire_subset, get_random_subset
...
class MDSS(object):
    ...
    def choose_aggregates(self, aggregates: dict, thresholds: list, penalty: float, all_observed_sum: float,
        ...
        for i in range(len(thresholds) - 1):
            ...
            current_interval_score = scoring_function.score(observed_sum, expectations, penalty * len(names),
                                                            current_q_mle)
    ...
    def scan(self, coordinates: pd.DataFrame, expectations: pd.Series, outcomes: pd.Series, penalty: float,
        ...
        for i in range(num_iters):
            ...
            subsets.append((current_subset, current_score))
        ...
        return best_subset, best_score, subsets
