...
from methods.subgroup.gerryfair.reg_oracle_class import RegOracle
...
class Group(object):
    ...
    def return_f(self):
        return [self.func, self.group_size, self.weighted_disparity, self.disparity, self.disparity_direction,
                self.group_rate]
    ...
    def to_dict(self):
        return {
            ...
            'weighted_disparity': self.weighted_disparity,
...
class Auditor:
    ...
    def __init__(self, X_prime, y, fairness_def):
        ...
        if self.fairness_def not in ['FP', 'FN']:
            raise Exception(
                'Invalid fairness metric specified: {}. Please choose \'FP\' or \'FN\'.'.format(self.fairness_def))
        self.y = self.y_input
    ...
    def get_subset(self, predictions):
        if self.fairness_def == 'FP':
            ...
            return self.X_prime_0, [(1 - a) for u, a in enumerate(predictions) if
                                    self.y[u] == 0]  # changed 1 to 0, inverting labels
