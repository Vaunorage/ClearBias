...
from methods.group.fairtest.modules.metrics import NMI, CondDIFF, CORR, CondNMI, CondCORR, DIFF
import logging
        ...
        for sens_str in self.sens_features:
            ...
            if sens_str in self.metrics:
                ...
                if expl is not None:
                    ...
                    # at least one feature is not categorical
                    if (sens.arity and sens.arity > 2) or \
                            (out.arity and out.arity > 2):
                        raise ValueError('No Metric available for continuous '
                                         'and multi-valued features')

                    # both numerical
                    if not sens.arity and not out.arity:
                        logging.info('Choosing metric CORR for feature %s'
                                     % sens_str)
                        self.metrics[sens_str] = CORR()
                    # one numerical, one categorical
                    else:
                        logging.info('Choosing metric DIFF for feature %s'\
                                     % sens_str)
                        self.metrics[sens_str] = DIFF()
