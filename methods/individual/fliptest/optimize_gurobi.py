...
def optimize(X1, X2, dists, counts1=None, counts2=None, decimals=6, verbose=True):
    num_rows1 = X1.shape[0]
    num_rows2 = X2.shape[0]
    if counts1 is None:
        counts1 = np.ones(num_rows1, dtype=np.int64)
    if counts2 is None:
        counts2 = np.ones(num_rows2, dtype=np.int64)
    num_ppl1 = np.sum(counts1)
    num_ppl2 = np.sum(counts2)

    # Normalize to create probability distributions
    a = counts1 / num_ppl1  # source distribution
    b = counts2 / num_ppl2  # target distribution
    ...
    # Organize results
    r12 = np.round(transport_plan * num_ppl1, decimals=decimals)
    forward = {}
    for i in range(num_rows1):
        forward[i] = {j: r12[i, j] for j in range(num_rows2) if r12[i, j] != 0}

    r21 = np.round(transport_plan.T * num_ppl2, decimals=decimals)
    reverse = {}
