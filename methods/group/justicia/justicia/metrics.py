from methods.group.justicia.justicia import utils
from methods.group.justicia.justicia.linear_classifier_wrap import linear_classifier_wrap_Wrapper, predict_lin
import itertools
from methods.group.justicia.justicia.ssat_wrap import Fairness_verifier
from methods.group.justicia.justicia.decision_tree_wrap import dtWrapper
import numpy as np
import pandas as pd
from methods.group.justicia.justicia import dependency_utils
from itertools import chain
from time import time
from sklearn.metrics import accuracy_score, f1_score

from methods.group.justicia.justicia.subsetsum import SubSetSumCount
...
class Metric:
    ...
    def __init__(self, model, data, sensitive_attributes, dependency_graph=None, mediator_attributes=[], major_group={},
                 encoding="Enum", filename="sample.sdimacs", verbose=True, feature_threshold=0, timeout=900,
                 discretization_bins=10, dag_threshold=0.001):
    ...
    def __repr__(self):
        if hasattr(self, '_model_name'):
            return "\nJusticia\n - model: {}\n".format(self._model_name) + '\n'.join(
                " - %s: %s" % (item, value) for (item, value) in vars(self).items() if
                (not item.startswith("_") and item != "model"))
        return "\nJusticia\n" + '\n'.join(
            " - %s: %s" % (item, value) for (item, value) in vars(self).items() if not item.startswith("_"))
        ...
        if (self._model_name == "linear-model"):
            ...
            for num_bins in range(1, self._discretization_bins):
                self._attributes, self._sensitive_attributes, self._probs, self._given_weights, self._variable_attribute_map, self._column_info, converted_df, self._discretized_group_attributes = utils.get_statistics_for_linear_classifier_wrap(
                    self._data.copy(), self._original_weights, self.given_sensitive_attributes, num_bins=num_bins,
            ...
            # get classifier
            lr = linear_classifier_wrap_Wrapper(weights=self._weights, attributes=self._attributes,
                                                sensitive_attributes=self._sensitive_attributes, bias=self._bias,
                                                converted_data=converted_df,
                                                original_model_prediction=self.model.predict(self._data),
                                                convert_to_cnf="Enum" in self.encoding, verbose=self._verbose)
    ...
    def _compute(self):
        ...
        if ("dependency" in self.encoding or self.encoding in ['Learn-correlation', 'Learn-efficient-correlation']):
                    ...
                    if (True):
                        ...
                        if (False):
                            edges, self._graph_edges, flag = dependency_utils._call_notears(self._transform(
                                self._data.copy()), self._sensitive_attributes, verbose=self._verbose,
                            ...
                            edges, self._graph_edges, flag = dependency_utils._call_pgmpy(
                                self._transform(self._data.copy(
            ...
            if ((self._model_name == "linear-model" and "Learn" in self.encoding)):
                ...
                dependency = dependency_utils.Dependency(edges, edge_weights, self._probs,
                                                         variable_header=self._num_attributes if self._num_attributes_neg is None else max(
                ...
                self._attributes = list(set(
                    [var for var in self._attributes if
                     var not in dependency.auxiliary_variables] + dependency.attributes))
            ...
            if (self.encoding == "Enum-dependency"):
                ...
                if (not ((self._model_name == "linear-model" and "Learn" in self.encoding))):
                    self._attributes_neg = list(set(
                        [var for var in self._attributes_neg if
                         var not in dependency.auxiliary_variables] + dependency.attributes))
    ...
    def _run_Enum(self, dependency_constraints=[]):
        ...
        for configuration in _combinations:
            ...
            fv.encoding_Enum_SSAT(self._classifier, self._attributes, self._sensitive_attributes,
                                  self._auxiliary_variables, self._probs, self._filename,
                                  dependency_constraints=dependency_constraints,
                                  sensitive_attributes_assignment=list(configuration), verbose=self._verbose)
    ...
    def _run_Learn_efficient(self, dependency_constraints=[], edge_weights={}):
        ...
        if (self._model_name == "linear-model"):
            ...
            fv = SubSetSumCount(weights=converted_weight, probs=self._probs, sum=-1 * self._linear_model.bias,
                                choice_variables=self._sensitive_attributes, conditional_probs=edge_weights,
                                verbose=self._verbose)
            fv.compute()
        ...
        fv.encoding_Learn_SSAT(self._classifier, self._attributes, self._sensitive_attributes,
                               self._auxiliary_variables, self._probs,
                               self._filename, dependency_constraints=dependency_constraints, find_maximization=True,
                               verbose=self._verbose)
        ...
        # minimum probability
        fv = Fairness_verifier(timeout=float(self._timeout / 2))
        fv.encoding_Learn_SSAT(self._classifier_neg, self._attributes_neg, self._sensitive_attributes_neg,
                               self._auxiliary_variables_neg,
                               self._probs, self._filename, dependency_constraints=dependency_constraints,
                               find_maximization=True, verbose=self._verbose)
        flag = fv.invoke_SSAT_solver(
    ...
    def _run_Learn(self, dependency_constraints=[]):
        ...
        fv.encoding_Learn_SSAT(self._classifier, self._attributes, self._sensitive_attributes,
                               self._auxiliary_variables, self._probs,
                               self._filename, dependency_constraints=dependency_constraints, find_maximization=False,
                               verbose=self._verbose)
        flag = fv.invoke_SSAT_solver(
            self._filename, find_maximization=False, verbose=self._verbose)
        if (flag == True):
    ...
    def _run_Enum_correlation(self):
        ...
        for configuration in _combinations:
            ...
            fv.encoding_Enum_SSAT(self._classifier, self._attributes, self._sensitive_attributes,
                                  self._auxiliary_variables,
                                  self._probs, self._filename, sensitive_attributes_assignment=list(configuration),
                                  verbose=self._verbose)
            flag = fv.invoke_SSAT_solver(self._filename, verbose=self._verbose)
