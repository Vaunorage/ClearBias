...
class Slicefinder(BaseEstimator, TransformerMixin):
    ...
    max_time: int, optional, default=60
        The maximum time in seconds to run the slice finding process. If `None`, there is
        no time limit.
    ...
    def _check_params(self):
        ...
        if self.max_time is not None and self.max_time <= 0:
            raise ValueError(f"Invalid 'max_time' parameter: {self.max_time}")
    ...
    def _search_slices(
        ...
        while (
            ...
            if self.max_time is not None and (time.time() - start_time) > self.max_time:
                logger.debug(f"Slicefinder timed out after {self.max_time} seconds.")
                break
